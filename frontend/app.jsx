    ...
    try {
      ...
      const moodRes = await fetch('/api/interpret-mood', {
        ...
        body: JSON.stringify({
          ...
          activity: '',
          music: '',
          avoid_explicit: avoidExplicit,
          genres: selectedGenres,
          scores: { energy: (energyLevel / 10) }
        })
      });
      if (!moodRes.ok) throw new Error('Failed to interpret mood');
      const moodSpec = await moodRes.json();

      // Stage 2: Searching tracks
      setStage('Searching for tracks...');
      const tracksRes = await fetch('/api/search-tracks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ spec: moodSpec, count: numTracks })
      });
      if (!tracksRes.ok) throw new Error('Failed to search tracks');
      const tracksJson = await tracksRes.json();
      const tracks = tracksJson.tracks || [];

      // Stage 3: Creating playlist
      setStage('Creating your playlist...');
      const playlistRes = await fetch('/api/create-playlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: `${moodInput} Vibes`,
          description: 'Generated by VibeChef',
          public: true,
          tracks
        })
      });
      const playlistJson = playlistRes.ok ? await playlistRes.json() : null;
      const playlistUrl = playlistJson?.url || '';
      const playlistName = playlistJson?.name || `${moodInput} Vibes`;
      ...
      setResults({
        ...
        tracks,
        moodSpec,
        summary: `Created a ${playlistName} playlist with ${tracks.length} tracks featuring ${moodSpec.genres.join(', ')}.`
      });
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ color: '#b8a0d6', fontSize: '0.9rem' }}>
                        {(track.artists && Array.isArray(track.artists)) ? track.artists.join(', ') : (track.artist || '')}
                      </div>
                    </div>
                  </div>
                ))}
